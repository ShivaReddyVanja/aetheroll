      ...
      {dateGroups.map(({ dateLabel, year, items: groupItems }) => {
        ...
        return (
          <section key={dateLabel} id={`year-${year}`} className="space-y-2">
            ...
            {/* Mobile: 3 items per row square grid. Desktop: Fluid Dynamic Natural Aspect Ratio Row Flow */}
            <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-1 sm:gap-1.5 items-start">
              {groupItems.map((item) => (
                <MediaCard
                  key={item.id}
                  item={item}
